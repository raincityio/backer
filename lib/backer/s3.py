...
class S3Remote:
    ...
    def _get_index_nodepath(self, fsguid, id_, name):
        return "%s/%s.meta" % (self._get_index_path(fsguid, id_), name)

    def put_data(self, metakey, stream):
        path = self._get_data_nodepath(metakey)
        logging.debug("s3 put %s to %s" % (metakey, path))
        self.s3.upload_fileobj(stream, self.bucket, path)
